...
pub fn scan(root: &Path, project_age_days: u64, config: Option<&CategoryConfig>, global_config: &Config) -> Result<CategoryResult> {
    ...
    let all_artifact_paths: Vec<PathBuf> = inactive_project_roots
        ...
        .flat_map(|project_root| {
            find_build_artifacts(project_root, &artifacts_to_scan)
        })
        .filter(|p| p.exists())
        .collect();
