...
fn fun_comparison(bytes: u64) -> Option<String> {
    ...
    if bytes >= 10 * GB {
        ...
        if count >= 1 {
            Some(format!("That's like ~{} AAA game installs (~{:.1} GB)!", count, gb))
        } else {
            Some(format!("That's like a partial game install (~{:.1} GB)!", gb))
        }
    } else if bytes >= 500 * MB {
        let count = bytes / node_modules_size;
        let gb = bytes as f64 / GB as f64;
        Some(format!("That's like ~{} node_modules folders (~{:.1} GB)!", count, gb))
    } else if bytes >= 10 * MB {
        let count = bytes / floppy_size;
        let mb = bytes as f64 / MB as f64;
        Some(format!("That's like ~{} floppy disks (~{:.0} MB)!", count, mb))
    } else {
        None
    }
}
...
pub fn render(f: &mut Frame, app_state: &mut AppState) {
    ...
    if selected_count == 0 {
        warning_lines.push(Line::from(vec![
            Span::styled("  ⚠  NO ITEMS SELECTED", Styles::warning()),
        ]));
        warning_lines.push(Line::from(vec![
            Span::styled("     Use Space to select items, then confirm deletion", Styles::secondary()),
        ]));
    } else {
        ...
        warning_lines.push(Line::from(vec![
            ...
            Span::styled(format!("{} {}", selected_count, item_text), Styles::emphasis()),
            Span::styled(format!(" ({})", bytesize::to_string(selected_size, true)), Styles::secondary()),
        ]));
        
        // Add fun comparison if applicable
        if let Some(comparison) = fun_comparison(selected_size) {
            warning_lines.push(Line::from(vec![
                Span::styled(format!("     {}", comparison), Styles::emphasis()),
            ]));
        } else {
        ...
        warning_lines.push(Line::from(vec![
            Span::styled("     Files will be moved to Recycle Bin (recoverable)", Styles::secondary()),
        ]));
    }
    ...
    let actions_lines = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(" (bypass Recycle Bin - cannot be undone!)", Styles::secondary()),
        ]),
    ];
...
fn render_summary_table(f: &mut Frame, area: Rect, app_state: &AppState) {
    ...
    for &index in &app_state.selected_items {
        if let Some(item) = app_state.all_items.get(index) {
            let entry = category_stats.entry(item.category.clone()).or_insert((0, 0));
            entry.0 += 1;
...
fn render_file_list(f: &mut Frame, area: Rect, app_state: &mut AppState) {
    ...
    if rows.is_empty() {
        let empty = Paragraph::new(Line::from(vec![
            Span::styled("  No items selected", Styles::secondary()),
        ]));
    ...
    for (row_idx, row) in rows.iter().enumerate() {
        ...
        match *row {
            crate::tui::state::ConfirmRow::CategoryHeader { cat_idx } => {
                ...
                let selected_in_group = item_indices.iter()
                ...
                lines.push(Line::from(vec![
                    ...
                    Span::styled(format!("{:>8}", bytesize::to_string(group.total_size, true)), Styles::primary()),
                    Span::styled("    ", Styles::secondary()),
                    Span::styled(format!("{}/{} items", selected_in_group, total_in_group), Styles::secondary()),
                    if group.safe {
            ...
            crate::tui::state::ConfirmRow::FolderHeader { cat_idx, folder_idx } => {
                ...
                let selected_in_folder = folder.items.iter()
                    .filter(|&&idx| app_state.selected_items.contains(&idx))
                    .count();
                ...
                lines.push(Line::from(vec![
                    ...
                    Span::styled(format!("  ({}/{})", selected_in_folder, total_in_folder), Styles::secondary()),
                ]));
                line_to_row.push(row_idx);
            }
    ...
    let cursor_line_idx = line_to_row.iter().position(|&r| r == app_state.cursor).unwrap_or(0);
