...
impl AppState {
    pub fn new() -> Self {
        ...
        let default_enabled: std::collections::HashSet<String> = config.categories.default_enabled
            .iter()
            .map(|s| s.to_lowercase())
            .collect();
        
        let all_category_names = vec![
            "Package cache", "Application cache", "Temp", "Trash", "Build", "Downloads", 
            "Large", "Old", "Browser", "System", "Empty", "Duplicates"
        ];
        ...
        let categories = all_category_names.iter().map(|name| {
            let enabled = if use_config_defaults {
                ...
                matches!(*name, "Package cache" | "Application cache" | "Temp" | "Trash" | "Build")
            };
            
            let description = match *name {
                "Package cache" => "Package manager caches (npm, pip, nuget, etc.)".to_string(),
                "Application cache" => "Application caches (Discord, VS Code, Slack, etc.)".to_string(),
                ...
                "Old" => format!("Files not accessed in {} days", config.thresholds.min_age_days),
    ...
    pub fn apply_config_to_state(&mut self) {
        ...
        for cat in &mut self.categories {
            match cat.name.as_str() {
                "Large" => cat.description = format!("Files over {}MB", self.config.thresholds.min_size_mb),
                "Old" => cat.description = format!("Files not accessed in {} days", self.config.thresholds.min_age_days),
                _ => {}
            }
        }
    }
    ...
    pub fn flatten_results(&mut self) {
        if let Some(ref results) = self.scan_results {
            ...
            let extract_parent_folder = move |path: &PathBuf| -> Option<String> {
                path.parent().map(|p| crate::utils::to_relative_path(p, &scan_path))
            };
            ...
            let find_project_root = |artifact_path: &PathBuf| -> Option<(PathBuf, String, bool)> {
                ...
                while let Some(parent) = current.parent() {
                    if crate::project::detect_project_type(current).is_some() {
                        ...
                        let is_active = crate::project::is_project_active(current, project_age_days).unwrap_or(false);
            ...
            let mut add_category = |paths: &[PathBuf], size_bytes: u64, category: &str, safe: bool| {
                ...
                if !paths.is_empty() {
                    ...
                    let folder_groups = if category == "Build" {
                        ...
                        let mut project_map: HashMap<(PathBuf, String, bool), Vec<usize>> = HashMap::new();
                        let mut ungrouped_items: Vec<usize> = Vec::new();

                        for &item_idx in &items {
                            if let Some(item) = self.all_items.get(item_idx) {
                                if let Some((project_path, project_name, is_active)) = find_project_root(&item.path) {
                                    // Group by project only (combine all artifact types)
                                    project_map.entry((project_path, project_name, is_active))
                                    ...
                                    if let Some(folder_name) = extract_parent_folder(&item.path) {
                                        ...
                                        project_map.entry((parent_path, folder_name, false))
                                            .or_insert_with(Vec::new)
                                            .push(item_idx);
                                    } else {
                        ...
                        let item_paths: Vec<(usize, &PathBuf)> = items.iter()
                            .filter_map(|&item_idx| {
                        ...
                        for (item_idx, path) in &item_paths {
                            ...
                            while let Some(parent) = current.parent() {
                                ...
                                dir_to_items.entry(parent_path.clone())
                                    .or_insert_with(Vec::new)
                                    .push(*item_idx);
                                current = parent_path;
                            }
                        ...
                        if let Some(ref common_parent) = best_common_parent {
                            ...
                            if !direct_items.is_empty() {
                                ...
                                folder_map.entry(folder_name).or_insert_with(Vec::new).extend(direct_items);
                            }
                            ...
                            for (subdir_path, subdir_items) in subfolder_map {
                                ...
                                folder_map.entry(folder_name).or_insert_with(Vec::new).extend(subdir_items);
                            }
                        } else {
                            ...
                            for (item_idx, path) in &item_paths {
                                if let Some(parent) = path.parent() {
                                    parent_to_items.entry(parent.to_path_buf())
                                        .or_insert_with(Vec::new)
                                        .push(*item_idx);
                                } else {
                                    ungrouped_items.push(*item_idx);
                            ...
                            let mut prefix_to_groups: HashMap<String, Vec<(PathBuf, Vec<usize>)>> = HashMap::new();
                                ...
                                if let Some(separator_pos) = parent_name.rfind('-') {
                                    if separator_pos > 0 && separator_pos < parent_name.len() - 1 {
                                        ...
                                        if suffix.chars().any(|c| c.is_ascii_digit()) {
                                            prefix_to_groups.entry(potential_prefix.to_string())
                                                .or_insert_with(Vec::new)
                                ...
                                if group_items.len() >= 2 {
                                    ...
                                    if let Some((first_parent, _)) = group_items.first() {
                                        if let Some(common_parent) = first_parent.parent() {
                                            ...
                                            let group_folder_name = common_parent_path.join(&prefix).display().to_string();
                                            ...
                                            folder_map.entry(group_folder_name)
                                                .or_insert_with(Vec::new)
                                            ...
                                            for (parent_path, items) in group_items {
                                                ...
                                                folder_map.entry(folder_name).or_insert_with(Vec::new).extend(items);
                                            }
                                        }
                                    } else {
                                        // Empty group, skip
                        ...
                        if let Some(ref common_parent) = best_common_parent {
                            ...
                            folder_groups.sort_by(|a, b| {
                                ...
                                let a_is_subfolder = a.folder_name.starts_with(&common_parent_str) && a.folder_name != common_parent_str;
                                let b_is_subfolder = b.folder_name.starts_with(&common_parent_str) && b.folder_name != common_parent_str;
            ...
            add_category(&results.cache.paths, results.cache.size_bytes, "Package cache", true);
            add_category(&results.app_cache.paths, results.app_cache.size_bytes, "Application cache", true);
            add_category(&results.temp.paths, results.temp.size_bytes, "Temp Files", true);
            add_category(&results.trash.paths, results.trash.size_bytes, "Trash", true);
            add_category(&results.build.paths, results.build.size_bytes, "Build Artifacts", true);
            add_category(&results.downloads.paths, results.downloads.size_bytes, "Old Downloads", false);
            add_category(&results.large.paths, results.large.size_bytes, "Large Files", false);
            add_category(&results.old.paths, results.old.size_bytes, "Old Files", false);
            add_category(&results.browser.paths, results.browser.size_bytes, "Browser Cache", true);
            add_category(&results.system.paths, results.system.size_bytes, "System Cache", true);
            add_category(&results.empty.paths, results.empty.size_bytes, "Empty Folders", true);
            add_category(&results.duplicates.paths, results.duplicates.size_bytes, "Duplicates", false);
                ...
                if group.grouped_by_folder {
                    ...
                    if group.folder_groups.is_empty() {
                        ...
                        for (folder_idx, folder_group) in group.folder_groups.iter().enumerate() {
                            rows.push(ResultsRow::FolderHeader { group_idx, folder_idx });
        ...
        for (group_idx, group) in self.category_groups.iter().enumerate() {
            ...
            if check_content {
                if group.grouped_by_folder {
                    ...
                    for (folder_idx, folder_group) in group.folder_groups.iter().enumerate() {
                        let folder_has_matches = folder_group.items.iter().any(|&idx| item_matches(idx));
                        
                        if folder_has_matches {
                            has_matching_items = true;
                            matching_rows.push(ResultsRow::FolderHeader { group_idx, folder_idx });
                            ...
                            if folder_idx < group.folder_groups.len() - 1 {
                                ...
                                let has_more_matches = group.folder_groups.iter()
                                    .enumerate()
                                    .skip(folder_idx + 1)
    ...
    pub fn confirm_rows(&self) -> Vec<ConfirmRow> {
        ...
        for (cat_idx, group) in confirm_groups.iter().enumerate() {
            ...
            if group.expanded {
                if group.grouped_by_folder {
                    for (folder_idx, folder_group) in group.folder_groups.iter().enumerate() {
                        rows.push(ConfirmRow::FolderHeader { cat_idx, folder_idx });
                        if folder_group.expanded {
                            for &item_idx in &folder_group.items {
    ...
    fn build_confirm_category_groups(&self) -> Vec<CategoryGroup> {
        ...
        for (category_name, item_indices) in category_vec {
            ...
            let original_group = self.category_groups.iter().find(|g| g.name == category_name);
            ...
            let total_size: u64 = item_indices.iter()
                .filter_map(|&idx| self.all_items.get(idx))
                .map(|item| item.size_bytes)
                .sum();
            ...
            let folder_groups = if grouped_by_folder {
                if category_name == "Build Artifacts" {
                    ...
                    let mut folders: Vec<FolderGroup> = project_map.into_iter()
                        .map(|((_, project_name, is_active), item_indices)| {
                            let folder_size: u64 = item_indices.iter()
                                .filter_map(|&idx| self.all_items.get(idx))
                                .map(|item| item.size_bytes)
                                .sum();
                    ...
                    if !ungrouped_items.is_empty() {
                        let ungrouped_size: u64 = ungrouped_items.iter()
                            .filter_map(|&idx| self.all_items.get(idx))
                            .map(|item| item.size_bytes)
                            .sum();
                    ...
                    let item_paths: Vec<(usize, &PathBuf)> = item_indices.iter()
                        .filter_map(|&item_idx| {
                            self.all_items.get(item_idx)
                                .map(|item| (item_idx, &item.path))
                        })
                    ...
                    // Check each directory level to find the best common parent
                    for (parent_path, items_in_parent) in &dir_to_items {
                        if items_in_parent.len() >= min_items_threshold && items_in_parent.len() > best_common_parent_count {
                            // Check if this parent is deeper (longer path) than current best
                            let is_better = if let Some(ref current_best) = best_common_parent {
                                // Prefer longer paths (deeper in hierarchy)
                                parent_path.components().count() > current_best.components().count() ||
                                (parent_path.components().count() == current_best.components().count() && 
                                 items_in_parent.len() > best_common_parent_count)
                            } else {
                                true
                    ...
                    let mut folders: Vec<FolderGroup> = folder_map.into_iter()
                        .map(|(folder_name, items)| {
                            let folder_size: u64 = items.iter()
                                .filter_map(|&idx| self.all_items.get(idx))
                                .map(|item| item.size_bytes)
                                .sum();
            ...
            groups.push(CategoryGroup {
                name: category_name,
                items: if grouped_by_folder { Vec::new() } else { item_indices },
                folder_groups,
                total_size,
    ...
    pub fn confirm_category_state(&self, category_name: &str) -> Option<(usize, bool)> {
        ...
        let mut confirm_groups: Vec<(String, Vec<usize>, bool)> = category_map
            .into_iter()
            .map(|(cat, items)| {
                let expanded = self.category_groups
                    .iter()
                    .find(|g| g.name == cat)
                    .map(|g| g.expanded)
                    .unwrap_or(true);
        ...
        confirm_groups.sort_by(|a, b| {
            let size_a: u64 = a.1.iter()
                .filter_map(|&idx| self.all_items.get(idx))
                .map(|item| item.size_bytes)
                .sum();
            let size_b: u64 = b.1.iter()
                .filter_map(|&idx| self.all_items.get(idx))
                .map(|item| item.size_bytes)
                .sum();
            size_b.cmp(&size_a)
    ...
    pub fn toggle_confirm_category(&mut self, category_name: &str) {
        if let Some(group) = self.category_groups.iter_mut().find(|g| g.name == category_name) {
    ...
    pub fn toggle_confirm_folder(&mut self, category_name: &str, folder_name: &str) {
        if let Some(group) = self.category_groups.iter_mut().find(|g| g.name == category_name) {
            if let Some(folder) = group.folder_groups.iter_mut().find(|f| f.folder_name == folder_name) {
    ...
    pub fn toggle_items(&mut self, item_indices: impl IntoIterator<Item = usize>) {
        ...
        let all_selected = all_related_indices.iter().all(|idx| self.selected_items.contains(idx));
        if all_selected {
    ...
    pub fn rebuild_groups_from_all_items(&mut self) {
        ...
        for (idx, item) in self.all_items.iter().enumerate() {
            by_category.entry(item.category.clone()).or_default().push(idx);
        }
        ...
        for (category, indices) in by_category.into_iter() {
            ...
            let folder_groups = if grouped_by_folder {
                ...
                // Collect all item paths
                let item_paths: Vec<(usize, &PathBuf)> = indices.iter()
                    .filter_map(|&item_idx| {
                        self.all_items.get(item_idx)
                            .map(|item| (item_idx, &item.path))
                    })
                ...
                if let Some(ref common_parent) = best_common_parent {
                    ...
                    for (item_idx, path) in &item_paths {
                        if let Some(item_parent) = path.parent() {
                            if item_parent == *common_parent {
                                ...
                                // Item is not under common parent - group separately
                                let folder_name = crate::utils::to_relative_path(item_parent, &scan_path);
                                folder_map.entry(folder_name).or_insert_with(Vec::new).push(*item_idx);
                            }
                        } else {
                            ungrouped_items.push(*item_idx);
                    ...
                    // Add direct items as a group under common parent
                    if !direct_items.is_empty() {
                        let folder_name = crate::utils::to_relative_path(common_parent, &scan_path);
                        folder_map.entry(folder_name).or_insert_with(Vec::new).extend(direct_items);
                    }
                    
                    // Add sub-folders as separate groups (they'll be displayed as children)
                    for (subdir_path, subdir_items) in subfolder_map {
                        let folder_name = crate::utils::to_relative_path(&subdir_path, &scan_path);
                        folder_map.entry(folder_name).or_insert_with(Vec::new).extend(subdir_items);
                    }
                } else {
                    // No common parent found - group by immediate parent, but detect similar prefixes
                        ...
                        if group_items.len() >= 2 {
                            ...
                            if let Some((first_parent, _)) = group_items.first() {
                                if let Some(common_parent) = first_parent.parent() {
                                    ...
                                    let group_folder_name = crate::utils::to_relative_path(&group_folder_path, &scan_path);
                                    ...
                                    // No common parent, add as standalone
                                    for (parent_path, items) in group_items {
                                        let folder_name = crate::utils::to_relative_path(&parent_path, &scan_path);
                                        folder_map.entry(folder_name).or_insert_with(Vec::new).extend(items);
                                    }
                                }
                            } else {
                ...
                // Sort folder groups: common parent first, then sub-folders, then others
                if let Some(ref common_parent) = best_common_parent {
                    let common_parent_str = crate::utils::to_relative_path(common_parent, &scan_path);
                    folder_groups.sort_by(|a, b| {
                        let a_is_common = a.folder_name == common_parent_str;
                        let b_is_common = b.folder_name == common_parent_str;
                        let a_is_subfolder = a.folder_name.starts_with(&common_parent_str) && a.folder_name != common_parent_str;
            ...
            self.category_groups.push(CategoryGroup {
                name: category,
                items: if grouped_by_folder { Vec::new() } else { indices },
                folder_groups,
                total_size,
                expanded: true,
                safe,
    ...
    pub fn sync_categories_to_config(&mut self) {
        ...
        self.config.categories.default_enabled = self.categories
            .iter()
