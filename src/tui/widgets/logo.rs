...
pub fn render_logo_animated(f: &mut Frame, area: Rect, progress: f32) {
    ...
    let title_lines: Vec<Line> = LOGO_LINES.iter().map(|line| {
...
pub fn render_logo_with_style(
    ...
    if animated {
        ...
        let title_lines: Vec<Line> = LOGO_LINES.iter().map(|line| {
            let chars: Vec<char> = line.chars().collect();
            let spans: Vec<Span> = chars.iter().enumerate().map(|(i, c)| {
        ...
        // Static logo
        let title_lines: Vec<Line> = LOGO_LINES.iter().map(|line| {
            Line::from(vec![Span::styled(*line, Styles::title())])
        }).collect();
