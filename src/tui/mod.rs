...
pub fn run(initial_state: Option<AppState>) -> Result<()> {
    ...
    loop {
        ...
        if matches!(app_state.screen, crate::tui::state::Screen::Restore { result: None }) {
        ...
        if scan_pending {
            ...
            if app_state.pending_action == crate::tui::state::PendingAction::Analyze {
                ...
                let scan_path = if let crate::tui::state::Screen::Scanning { ref progress } = app_state.screen {
        ...
        if event::poll(Duration::from_millis(100))? {
            ...
            match event::read()? {
                ...
                Event::Mouse(mouse) => {
                    match handle_mouse_event(&mut app_state, mouse) {
                        events::EventResult::Quit => break,
                        events::EventResult::Continue => {}
                    }
                }
...
fn perform_scan_with_progress(
    ...
    let enabled_categories: Vec<String> = app_state.categories
        .iter()
            ...
            if let crate::tui::state::Screen::Scanning { ref mut progress } = app_state.screen {
                ...
                for cat_progress in &mut progress.category_progress {
                    if cat_progress.name == *cat_name {
                        ...
                        cat_progress.progress_pct = ((idx + 1) as f32 / (total_categories + 1) as f32) * 0.9;
                    }
                }
            }
        }
...
fn perform_cleanup(
    ...
    let mut cleaned_bytes = if trash_cleaned > 0 { trash_total_bytes } else { 0 };
    ...
    for (_item_idx, category, path, size_bytes) in items_to_clean {
        ...
        let delete_result = match category.as_str() {
            "Browser" => {
                categories::browser::clean(&path)
            }
            "System" => {
                categories::system::clean(&path)
            }
            "Empty" => {
                categories::empty::clean(&path)
            }
            _ => {
