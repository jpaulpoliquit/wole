...
/// System directories to skip during old file scanning
...
/// 
/// Scans Downloads, Documents, and Desktop folders for files not modified
/// in the specified number of days. Skips system directories and active project directories.
pub fn scan(_root: &Path, min_age_days: u64) -> Result<CategoryResult> {
    ...
    for dir in user_dirs {
        scan_directory(&dir, &cutoff, &mut result, &mut paths)?;
    }
...
/// Scan a directory for old files, skipping system dirs and active projects
fn scan_directory(
    dir: &Path,
        ...
        .filter_map(|e| e.ok())
    {
        ...
        // Check if it's a file and meets age threshold
