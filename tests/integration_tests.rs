...
fn test_long_path_conversion() {
    ...
    #[cfg(windows)]
    {
        ...
        assert!(path_str.starts_with(r"\\?\"), "Path should start with \\\\?\\");
    }
...
fn test_deletion_log_mixed_results() {
    ...
    log.log_failure(std::path::Path::new("/locked.txt"), 500, "cache", false, "Locked");
...
fn test_deletion_record_success() {
    let record = DeletionRecord::success(
        std::path::Path::new("/test/file.txt"),
        1024,
        "cache",
        false,
    );
