...
use crate::history::{load_log, list_logs, DeletionLog};
...
pub fn restore_last(output_mode: crate::output::OutputMode) -> Result<RestoreResult> {
    ...
    if logs.is_empty() {
        return Err(anyhow::anyhow!("No deletion history found. Nothing to restore."));
    }
...
pub fn restore_from_log(log: &DeletionLog, output_mode: crate::output::OutputMode) -> Result<RestoreResult> {
    let mut result = RestoreResult::default();
    
    // Get current Recycle Bin contents
    let recycle_bin_items = os_limited::list()
    ...
    let mut bin_map: std::collections::HashMap<String, &trash::TrashItem> = std::collections::HashMap::new();
    ...
    for record in &log.records {
        ...
        if let Some(trash_item) = bin_map.get(&path_str) {
            match restore_file(trash_item) {
                Ok(()) => {
                    ...
                    if output_mode != crate::output::OutputMode::Quiet {
                        println!("{} Restored: {}", Theme::success("✓"), Theme::secondary(&record.path));
                    }
                }
                Err(e) => {
                    result.errors += 1;
            ...
            if output_mode == crate::output::OutputMode::VeryVerbose {
                println!("{} Not found in Recycle Bin: {}", 
                    Theme::muted("?"), 
                    Theme::secondary(&record.path)
                );
            }
...
pub fn restore_path(path: &Path, output_mode: crate::output::OutputMode) -> Result<RestoreResult> {
        ...
        if original_path.display().to_string().to_lowercase() == path_str {
            match restore_file(item) {
                Ok(()) => {
                    result.restored = 1;
                    if output_mode != crate::output::OutputMode::Quiet {
                        println!("{} Restored: {}", Theme::success("✓"), Theme::secondary(&path.display().to_string()));
                    }
                    return Ok(result);
                }
                Err(e) => {
                    return Err(anyhow::anyhow!("Failed to restore {}: {}", path.display(), e));
                }
            }
        }
    }
    ...
    for item in &recycle_bin_items {
        let original_path = item.original_parent.join(&item.name);
        if original_path.display().to_string().to_lowercase().contains(&path_str) ||
           path_str.contains(&original_path.display().to_string().to_lowercase()) {
            match restore_file(item) {
                Ok(()) => {
                    result.restored = 1;
                    if output_mode != crate::output::OutputMode::Quiet {
                        println!("{} Restored: {}", Theme::success("✓"), Theme::secondary(&original_path.display().to_string()));
                    }
                    return Ok(result);
                }
                Err(e) => {
                    return Err(anyhow::anyhow!("Failed to restore {}: {}", original_path.display(), e));
                }
            }
        }
    }
    
    Err(anyhow::anyhow!("File not found in Recycle Bin: {}", path.display()))
}
