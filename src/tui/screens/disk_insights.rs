...
pub fn render(f: &mut Frame, app_state: &mut AppState) {
    ...
    // Render content
    render_content(f, chunks[2], &insights_clone, &current_path_clone, cursor, sort_by, app_state);
...
fn render_header(f: &mut Frame, area: Rect, insights: &crate::disk_usage::DiskInsights, current_path: &std::path::Path) {
    ...
    let header_text = format!("{}  |  Total: {}  |  {} files",
        breadcrumb_str,
...
fn render_content(f: &mut Frame, area: Rect, insights: &crate::disk_usage::DiskInsights, current_path: &std::path::Path, cursor: usize, sort_by: SortBy, app_state: &AppState) {
    // Find current folder node
    let current_node = find_folder_by_path(&insights.root, current_path)
        .unwrap_or(&insights.root);
    ...
    for (i, child) in children.iter().enumerate() {
        ...
        let line = Line::from(vec![
            ...
            Span::raw(" "),
            Span::styled(bar_filled, if is_selected { Styles::selected() } else { Styles::emphasis() }),
    ...
    for (i, file) in files.iter().enumerate() {
        ...
        let pct_str = if current_node.size > 0 {
            format!("{:.1}%", (file.size as f64 / current_node.size as f64) * 100.0)
        } else {
            "0.0%".to_string()
        };
